...
public class DmnTemplateServiceImpl implements DmnTemplateService {
  ...
  public DmnTemplateServiceImpl(
      DmnTemplateRepository dmnTemplateRepository, DmnService dmnService) {
  ...
  private DecisionTableModel convertDecisionTable(Sheet sheet) {
    ...
    Assert.isTrue(
        "INPUTS".equalsIgnoreCase(headerTypeRow.getCell(inputHeaderIndex).getStringCellValue()),
        "No INPUTS header section can be found");
    Assert.isTrue(
        "OUTPUTS".equalsIgnoreCase(headerTypeRow.getCell(outputHeaderIndex).getStringCellValue()),
